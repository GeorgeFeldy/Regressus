...
namespace Regressus.NPCs.Bosses.Oracle
{
    public class TheOracle : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            {
                Projectile.NewProjectile(NPC.GetSpawnSource_ForProjectile(), (Main.MouseWorld.Distance(arenaCenter) < (1322 / 2) * 2.5f ? Main.MouseWorld : arenaCenter), Vector2.Zero, ModContent.ProjectileType<OracleRoDRift>(), 0, 0, player.whoAmI);
            }
            if (AIState == Intro)
            {
                AITimer++;
                if (AITimer == 1)
                {
                    crystal = Main.npc[NPC.NewNPC(NPC.GetSpawnSourceForNPCFromNPCAI(), (int)NPC.Center.X, (int)NPC.Center.Y, ModContent.NPCType<OracleCrystal>(), ai0: NPC.whoAmI, Target: player.whoAmI)];
                    if (player.ownedProjectileCounts[ModContent.ProjectileType<OracleOrbs>()] <= 0)
                    {
                        for (int i = 0; i < 4; i++)
                        {
                            Projectile projectile = Main.projectile[Projectile.NewProjectile(NPC.GetSpawnSourceForProjectileNPC(), NPC.Center, Vector2.Zero, ModContent.ProjectileType<OracleOrbs>(), 0, 0, Main.myPlayer, NPC.whoAmI)];
            ...
            else if (AIState == Transform)
            {
                ...
                if (transformFrameCounter == 80)
                {
                    for (int i = 0; i < 15; i++)
                    {
                        ...
                        Projectile projectile = Main.projectile[Projectile.NewProjectile(NPC.GetSpawnSource_ForProjectile(), NPC.Center, velocity, ModContent.ProjectileType<TentacleBlack>(), 0, 0f, player.whoAmI, 200, .75f)];
            ...
            else if (AIState == BeamStar)
            {
                ...
                if (AITimer == 10)
                {
                    for (int k = 0; k < 5; k++)
                    {
                        ...
                        int projectile = Projectile.NewProjectile(NPC.GetSpawnSource_ForProjectile(), NPC.Center, velocity, ModContent.ProjectileType<OracleTelegraphLine>(), 0, 0, player.whoAmI, NPC.whoAmI, 1);
                ...
                if (AITimer == 40)
                {
                    for (int k = 0; k < 5; k++)
                    {
                        float angle = 2f * (float)Math.PI / 5f * k;
                        Vector2 velocity = new Vector2(15, 15).RotatedBy(angle);
                        int projectile = Projectile.NewProjectile(NPC.GetSpawnSource_ForProjectile(), NPC.Center, velocity, ModContent.ProjectileType<OracleBeamNoTarget>(), 45, 0, player.whoAmI, 0, 0.89f);
                        Main.projectile[projectile].tileCollide = false;
                    }
                }
                if (AITimer >= 70 && AITimer <= 340)
                {
                    ...
                    {
                        Projectile.NewProjectile(NPC.GetSpawnSource_ForProjectile(), NPC.Center, velocity * 2.5f, ModContent.ProjectileType<OracleEnergyOrb>(), 45, 0);
                    }
                }
                if (AITimer >= 400)
                {
            ...
            else if (AIState == GroundLasers)
            {
                ...
                if (AITimer2 == 10)
                {
                    ...
                    for (int i = 0; i < 6; i++)
                    {
                        ...
                        int projectilea = Projectile.NewProjectile(NPC.GetSpawnSource_ForProjectile(), random[i], -Vector2.UnitY, ModContent.ProjectileType<OracleTelegraphLine>(), 0, 0, player.whoAmI, NPC.whoAmI);
                        Main.projectile[projectilea].timeLeft = 36;
                    }
                    int projectile = Projectile.NewProjectile(NPC.GetSpawnSource_ForProjectile(), random[6], -Vector2.UnitY, ModContent.ProjectileType<OracleTelegraphLine>(), 0, 0, player.whoAmI, NPC.whoAmI);
                ...
                if (AITimer2 == 45)
                {
                    ...
                    for (int i = 0; i < 6; i++)
                    {
                        int projectilea = Projectile.NewProjectile(NPC.GetSpawnSource_ForProjectile(), random[i], -Vector2.UnitY, ModContent.ProjectileType<OracleBeam>(), 45, 0, player.whoAmI, 0, 1.875f);
                        Main.projectile[projectilea].timeLeft = 20;
                    }
                    int projectile = Projectile.NewProjectile(NPC.GetSpawnSource_ForProjectile(), random[6], -Vector2.UnitY, ModContent.ProjectileType<OracleBeam>(), 45, 0, player.whoAmI, 0, 1.875f);
