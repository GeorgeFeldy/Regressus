            ...
            if (NPC.ai[1] == 3)
            {
                ...
                if (aiTimer == 40)
                {
                    ...
                    for (int i = (projectileNum - projectileNum - projectileNum); i <= projectileNum; i++)
                    {
                        ...
                        Projectile projectile = Main.projectile[Projectile.NewProjectile(NPC.GetSpawnSource_ForProjectile(), NPC.Center, 1.5f * vel, ModContent.ProjectileType<OracleTelegraphLine>(), 0, 0, player.whoAmI, NPC.whoAmI, 1)];
